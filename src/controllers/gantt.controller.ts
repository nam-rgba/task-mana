...
class GanttController {
	...
	updateSchedule = async (req: Request, res: Response, next: NextFunction) => {
		...
		const result = await ganttService.updateSchedule(projectId, taskUpdates || [], scheduleUpdates || [], autoSchedule ?? true)
	...
	addDependency = async (req: Request, res: Response, next: NextFunction) => {
		...
		new CreatedResponse('Dependency added successfully!', 201, await taskDependencyService.addDependency(successorId, req.body)).send(
			res
		)
	}
